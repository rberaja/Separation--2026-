...
import { PROPERTY_CALC_COLUMNS, PROPERTY_INPUT_COLUMNS, buildReport, isoDate, reportFileStem, settlementSentence } from '../src/lib/report';
...
const props = [
  createProperty(1, { name: 'A1', assign: 'a', marketVal: 1000000, capex: 100000, noi: 60000, loanBal: 400000, loanRate: 5, loanTerm: 5, bidDiff: 10000, residualBasis: 500000, zoning: 'RM-24', cert40yr: '2031' }),
...
function report(cash = 250000) {
  ...
  return buildReport({ properties: props, settlement, partnerNames: names, pctA: 0.4, discountRate: 6.5, cashEquiv: cash });
}
...
describe('buildReport', () => {
  it('carries settings, counts and one row per property with input + calculated cells', () => {
    ...
    for (const c of [...PROPERTY_INPUT_COLUMNS, ...PROPERTY_CALC_COLUMNS]) expect(row.cells).toHaveProperty(c.key);
    expect(row.cells['residual_tax_basis']).toBe(500000);
  ...
  it('leaves the Residual Basis True-Up blank and names the tool that supplies it', () => {
...
describe('buildReportWorkbook', () => {
  ...
  it('round-trips through parseWorkbook, restoring every property and the settings', () => {
    ...
    expect(properties[0]).toMatchObject({
      ...
      loanRate: 5, loanTerm: 5, bidDiff: 10000, residualBasis: 500000, zoning: 'RM-24', cert40yr: '2031',
    });
    expect(properties[2]).toMatchObject({ name: 'Loose', assign: 'none', marketVal: 250000 });
    expect(meta).toEqual({ partnerAName: 'Roberto', partnerBName: 'Alex', partnerAPct: 40, discountRate: 6.5, cashEquiv: 250000 });
  });

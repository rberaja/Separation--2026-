...
<span style="color:#FFFFFF">- **AppFolio Project Directory mapping:** accept the native Project Directory CapEx export. Map `Name` to the CapEx description and `Total Budget` to the approximate cost. Use the report metadata date — **As of**, **Exported On**, or **Report Date** — as the item’s As Of date when there is no row-level date, while retaining the worksheet row as the source reference.</span>
...
<span style="color:#00A651">- **Consolidated partner selection:** place Partner A / Partner B controls on each consolidated group header, not on its member cards. A group selection assigns or unassigns every member together; an ungrouped property remains its own selection unit. Count each consolidated group as one assigned property in partner totals, the unassigned indicator, and exports.</span>
<span style="color:#00A651">- **Partition action bar:** show **Expand All**, **Collapse All**, and all Sort controls in the top action row beside Print / Export and Clear All. Expand / Collapse applies to every consolidated group and ungrouped property card. Keep Upload Excel, Column Guide, and Download Template available in the codebase but hidden from this action row until they are needed again.</span>
<span style="color:#00A651">- **Cash & Equivalents stability:** the Cash & Equivalents setting is independent of property assignment and remains at its entered value until the user enters a replacement amount.</span>
